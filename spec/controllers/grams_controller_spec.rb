...
RSpec.describe GramsController, type: :controller do
  ...
  describe "grams#destroy action" do
    ...
    it "shouldn't let unathenicated users destroy a gram" do
      gram = FactoryBot.create(:gram)
      delete :destroy, params: { id: gram.id }
      expect(response).to redirect_to new_user_session_path
    end
    ...
    it "should return a 404 messgae is we cannot find a gram with the id that is specified" do
      user = FactoryBot.create(:user)
      sign_in user
  ...
  describe "grams#edit" do
    ...
    it "shouldn't let unauthenticaed users edit a gram" do
      gram = FactoryBot.create(:gram)
      get :edit, params: { id: gram.id }
      expect(response).to redirect_to new_user_session_path
    end
    ...
    it "should return a 404 error message if thegram is not found" do
      user = FactoryBot.create(:user)
      sign_in user
  ...
  describe "grams#index action" do
    it "should successfully show you the page" do
      get :index
      expect(response).to have_http_status(:success)
    end
  end
